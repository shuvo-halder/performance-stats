...
export default function ServersPage() {
  ...
  const handleDelete = async (id, hostname) => {
    if (!window.confirm(`Remove server "${hostname}"? This will delete all associated data.`)) return;
    try {
          ...
          <div className="panel-body">
            {servers.length === 0 ? <div className="chart-placeholder">No servers registered</div> : (
              <div className="grafana-table">
                <div className="grafana-table-header" style={{gridTemplateColumns:'1.5fr 1fr 1fr 1fr 80px 100px 80px'}}>
                  <span>Hostname</span><span>IP</span><span>OS</span><span>Agent</span><span>Status</span><span>Last Seen</span><span>Actions</span>
                </div>
                {servers.map(s => (
                  <div key={s.id} className="grafana-table-row" style={{gridTemplateColumns:'1.5fr 1fr 1fr 1fr 80px 100px 80px'}}>
                    ...
                    <span><button className="mode-btn" style={{color:'#ef4444'}} onClick={() => handleDelete(s.id, s.hostname)}>Remove</button></span>
                  </div>
                ))}
              </div>
            )}
